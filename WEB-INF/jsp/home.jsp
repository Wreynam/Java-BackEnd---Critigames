    ...
    <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond&family=Inika&family=Karantina&display=swap" rel="stylesheet">
	 ...
	 <body>
		...
		<section class="banner">
			...
			<div class="carousel-inner">
				...
				<div class="carousel-item active">
					<img src="./img/dmc5.png" class="d-block w-100" alt="...">
				</div>
				<div class="carousel-item">
					<img src="./img/control.png" class="d-block w-100" alt="...">
				</div>
			</div>
		...
		<section class="noticias">
			<h1>NOTICIAS DE GAMES</h1>
			<div class="sclick-carrousel">
				<c:if test="${tamanhoNoticia > 0}">
					<c:forEach var="cout" begin="0" end="${tamanhoNoticia - 1}">
						<a class="noticiagames" href="entrada?acao=noticia&idnoticia=${listaNoticia[cout].getCodigo()}">
							<div class="textnews">
								<p>${listaNoticia[cout].getTitulo()}</p>
								<h1>${listaNoticia[cout].getData()}</h1>
							</div>
						</a>
					</c:forEach>
				</c:if>
		...
		<section class="games">
			...
			<div class="sclick-carrousel">
				<c:if test="${tamanho > 0}">
					<c:forEach var="couter" begin="0" end="${tamanho - 1}">
						<a href="entrada?acao=gamePage&idgamePage=${listaCodigo[couter]}" style="background-image: url('imagens/jogos/${ listaPasta[couter]}/${listaNomeMiniatura[couter]}'); width: 194px; height: 259px; background-size: cover;" >
							<div class="score" style="background-color: <c:if test="${listaGamePage[couter].getCritigamesScore() <= 50 }"> red</c:if>
									<c:if test="${listaGamePage[couter].getCritigamesScore() < 70 && listaGamePage[couter].getCritigamesScore() > 50}"> yellow </c:if>
									<c:if test="${listaGamePage[couter].getCritigamesScore() >= 70}"> green </c:if>;">
								<p 
								>${listaGamePage[couter].getCritigamesScore()}</p>
							</div>
						</a>
					</c:forEach>
				</c:if>
		...
		<section class="noticiasGeral">
			<h1>NOTICIAS</h1>
			<div class="sclick-carrousel">
				<c:if test="${tamanhoGeral > 0}">
					<c:forEach var="counter" begin="0" end="${tamanhoGeral - 1}">
						<a href="entrada?acao=noticia&idnoticia=${listaNoticiaGeral[counter].getCodigo()}" style="background-image: url('imagens/noticiasGeral/${listaNoticiaGeral[counter].getMiniFoto()}');" class="news" >
							<p>${listaNoticiaGeral[counter].getTitulo()}</p>
						</a>
					</c:forEach>
				</c:if>
			</div>
		...
		<section class="lojas">
			<h1>LOJAS</h1>
			<div class="sclick-carrousel">
				<a href="https://store.steampowered.com/" target="_blank" rel="noopener noreferrer"><img src="img/storesteam.png" alt=""></a>
				<a href="https://store.epicgames.com/pt-BR/" target="_blank" rel="noopener noreferrer"><img src="img/storeepic.png" alt=""></a>
				<a href="https://www.gog.com/" target="_blank" rel="noopener noreferrer"><img src="img/storegog.png" alt=""></a>
				<a href="https://www.blizzard.com/pt-br/" target="_blank" rel="noopener noreferrer"><img src="img/storenet.png" alt=""></a>
			</div>
		</section>
		<footer class="footer">
